...
function buildExternalAgentSystemContext({ mode, chatSessionId, defaultTargetSession, userSkillsContext }) {
  ...
  return appendVaultAgentGuidance(
    ...
    `For simple read-only requests such as current host system resources, CPU, memory, disk, load, hostname, IP address, pwd, whoami, uname, or process checks, do not answer with greetings, acknowledgements, or follow-up questions. Immediately call terminal_execute against the selected session, then report the command output. ` +
